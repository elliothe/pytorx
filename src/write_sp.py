...
with open(filename, 'w') as f:
    ...
    for i in range(crxb_size):
        ...
        f.write("rin{0} input{0} top{0}0 ".format(i) + str(1/gloaddac) + "\n")

    # writing rest nodes
    for i in range(crxb_size):
        for j in range(crxb_size):
            if i > 0 and j > 0:
                f.write("rrow{0}{1} top{0}{2} top{0}{1} ".format(i, j, j - 1) + str(1 / gwire) + "\n")
                f.write("rcol{0}{1} bot{2}{1} bot{0}{1} ".format(i, j, i - 1) + str(1 / gwire) + "\n")
                f.write("r{0}{1} top{0}{1} bot{0}{1} ".format(i, j) + str(1 / gmat) + "\n")
            elif i == 0 and j > 0:
                f.write("rrow{0}{1} top{0}{2} top{0}{1} ".format(i, j, j - 1) + str(1 / gwire) + "\n")
                f.write("r{0}{1} top{0}{1} bot{0}{1} ".format(i, j) + str(1 / gmat) + "\n")
            elif i > 0 and j == 0:
    ...
    for i in range(crxb_size):
        f.write("rout{1} 0 bot{0}{1} ".format(crxb_size-1, i) + str(1 / gloadadc) + "\n")
    f.write("\n")
